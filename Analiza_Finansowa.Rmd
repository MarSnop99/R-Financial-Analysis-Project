...
formatted_table <- final_report_table %>%
  mutate(across(where(is.numeric) & !contains("Ratio") & !contains("ROE") & !contains("ROA"), ~ comma(.x, accuracy = 1))) %>%
  mutate(across(contains("Ratio") | contains("ROE") | contains("ROA"), ~ number(.x, accuracy = 0.01)))

knitr::kable(formatted_table, caption = "Kluczowe Wskaźniki Finansowe dla AAPL")
```
